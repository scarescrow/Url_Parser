<?php
	...
	*/
	...
	function clean_irregularities($str) {
		...
		$str = str_replace("<em><strong>Singly Linked List:</strong></em>","",$str);
		$str = str_replace("<em><strong>Circular Linked List: </strong></em>","",$str);
		$str = str_replace("<em><strong>Doubly Linked List:</strong> </em>","",$str);
		return $str;
	...
	function insert_db($topic, $link, $subtopic) {
	
		global $con;
		$query = "INSERT INTO data_structures (Topic, Subtopic, Link) VALUES ('$topic', '$subtopic', '$link');";
	...
	while(strpos($main_section, '</p>') !== false) {
		...
		*/
		...
		while(strpos($critical_section, '<a') !== false) {
			...
			//Now that we have topic and link, insert to db
			
			insert_db($topic, $link, $subtopic);
